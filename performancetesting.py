...
query3_sf = "SELECT DAYOFWEEK(apptdate), D.region_name, D.province, D.city, AVG(D.appts) FROM 	(SELECT l.region_name, l.province, l.city, DATE(a.queuedate) as apptdate, COUNT(a.apptid) AS appts FROM appointments a JOIN clinics c ON a.clinicid = c.clinicid JOIN locations l ON c.locationid = l.locationid WHERE a.apptstatus = \"Complete\" OR a.apptstatus = \"Queued\" OR a.apptstatus = \"Serving\" GROUP BY DATE(a.queuedate), l.region_name, l.province, l.city) D GROUP BY DAYOFWEEK(apptdate), D.region_name, D.province, D.city WITH ROLLUP ORDER BY DAYOFWEEK(apptdate), AVG(D.appts) DESC, D.region_name, D.province, D.city;"
query3_rf = "SELECT DAYOFWEEK(apptdate), D.region_name, D.province, D.city, AVG(D.appts) FROM (SELECT c.city, c.province, c.region_name, DATE(a.queuedate) as apptdate, COUNT(*) AS appts FROM (SELECT clinicid, queuedate FROM appointments WHERE apptstatus = \"Complete\" OR apptstatus = \"Queued\" OR apptstatus = \"Serving\") a JOIN clinics c ON a.clinicid = c.clinicid GROUP BY DATE(a.queuedate), c.region_name, c.province, c.city) D GROUP BY DAYOFWEEK(apptdate), D.region_name, D.province, D.city WITH ROLLUP ORDER BY DAYOFWEEK(apptdate), AVG(D.appts) DESC, D.region_name, D.province, D.city;"
